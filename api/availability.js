...
module.exports = async (req, res) => {
  ...
  try {
    ...
    let calendarBusyEvents = [];
    let calendarMeta = { configured: false, fetchedAt: null, fromCache: false };

    try {
      const calendarResult = await getCalendarBusyEvents({
        forceRefresh,
        days,
        timeZone: STUDIO_TIME_ZONE,
        zonedTimeToUtc
      });
      calendarBusyEvents = calendarResult.events || [];
      calendarMeta = {
        configured: calendarResult.configured,
        fetchedAt: calendarResult.fetchedAt,
        fromCache: calendarResult.fromCache,
        stale: Boolean(calendarResult.stale)
      };
    } catch (error) {
      console.error("calendar load error:", error);
      return res.status(503).json({
        error:
          "Unable to load lesson calendar. Please try again in a moment or contact the studio."
      });
    }

    if (calendarMeta.configured && !calendarBusyEvents.length && !calendarMeta.fromCache) {
      console.warn("calendar configured but returned no busy events for the requested range");
    }

    if (!calendarMeta.configured) {
      console.warn(
        "MMS_ICAL_URL is not set; availability ignores existing lessons until the calendar URL is configured"
      );
    }
    ...
    for (const dateKey of dateKeys) {
      ...
      for (
        ...
        const slot = buildSlotForLocalDate({
          dateKey,
          startMinute,
          duration
        });
