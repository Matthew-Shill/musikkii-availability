...
function renderCalendar() {
  ...
  grid.innerHTML = gridDays.map(date => {
    ...
    const dateKey = getDateKeyForTimeZone(noon, selectedTimeZone);
...
async function loadAvailability(duration = 30, { refresh = false } = {}) {
  selectedDuration = duration;
  selectedDate = "";
  selectedTime = "";
  ...
  const res = await fetch(`/api/availability?duration=${duration}&days=42${refreshParam}`);
...
function bindStaticEvents() {
  ...
  document.getElementById("request-form").addEventListener("submit", async e => {
    ...
    try {
      ...
      selectedTime = "";
      updateSelectionCard();
      await loadAvailability(selectedDuration, { refresh: true });
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.addEventListener("visibilitychange", () => {
    if (document.visibilityState === "visible" && !isSubmitting) {
      loadAvailability(selectedDuration, { refresh: true });
    }
  });
});
